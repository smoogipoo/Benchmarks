...
namespace BenchmarksProject
{
    ...
    public class BenchmarkThreadedTaskScheduler
    {
        [Params(1)]
        ...
        public void Idle()
        {
            using (var scheduler = new ThreadedTaskScheduler(Threads, "Idle"))
                Thread.Sleep(100);
        }
        ...
        {
            using (var scheduler = new ThreadedTaskScheduler(Threads, "Saturated"))
            {
                for (int i = 0; i < Threads; i++)
                    Task.Factory.StartNew(() => Thread.Sleep(100), CancellationToken.None, TaskCreationOptions.None, scheduler);
            }
        }

        [Benchmark]
        public void SuperSaturated()
        {
            using (var scheduler = new ThreadedTaskScheduler(Threads, "SuperSaturated"))
            {
